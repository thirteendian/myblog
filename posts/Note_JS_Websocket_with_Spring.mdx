...
## Websocket With Java Springboot
...
## Message, Server, Servlet, Request, Response
...
*Servlets* run in a servlet container, which the well known one might be **Tomcat**. It is a Java object to handle Http
Request.
...
Apache's.  `ServletRequest` and `ServletResponse` provide three part: *HTTP Servlet*, *Body* and *Headers*.

> `HttpServletRequest` =(extends)= `ServletRequest`

> `HttpServletResponse` =(extends)= `ServletResponse`

Similary, two other things `HttpInputMessage` and `HttpOutputMessage` only has *Body* and *Headers*,
...
## Springboot Basic Websocket Configuration
...
## Springboot EventListener and HeadAccessor
